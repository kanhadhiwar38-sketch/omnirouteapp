...
import type { ProviderType, ModelProvider } from "@/types";
import {
  useSettingsStore,
  formatModelName,
  getEffectiveBaseUrl,
...
function getProviderApiKeyHelpUrl(
  ...
  if (provider?.baseUrl) {
    ...
    const matchedPreset = presets.find(
      (p) =>
        provider.baseUrl.startsWith(p.baseUrl) ||
        p.baseUrl.startsWith(provider.baseUrl),
    );
    if (matchedPreset) {
      return matchedPreset.apiKeyHelpUrl;
    }
  }
...
const ProviderSettings = () => {
  ...
  const providerApiKeyHelpUrl = getProviderApiKeyHelpUrl(
  ...
  const handleAddProviderFromPreset = (preset: ProviderPreset) => {
    const newId = addProviderFromPreset(preset);
    setSelectedProviderId(newId);
  };
            ...
            <div className="flex items-center gap-2">
              {getProviderPresets().map((preset) => (
                <Button
                  ...
                  variant="bare"
                  type="button"
                  onClick={() => handleAddProviderFromPreset(preset)}
                  className="text-xs px-2.5 py-1.5 border border-gray-300 dark:border-border rounded-lg bg-white dark:bg-muted text-gray-600 dark:text-muted-foreground hover:border-gray-400 dark:hover:border-gray-500 transition-colors"
                  aria-label={`Add ${preset.name} provider`}
                >
                  <Plus size={12} aria-hidden="true" /> {preset.name}
                </Button>
              ))}
              <Button
                variant="bare"

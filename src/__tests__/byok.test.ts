...
describe("BYOK secret envelopes", () => {
  ...
  it("builds OpenAI Compatible BYOK envelopes for OpenRouter presets", async () => {
    vi.resetModules();
    resetByokKeyMaterial();
    const {
      decryptSecretEnvelope: freshDecryptSecretEnvelope,
    ...
    const apiKeySecret = await encryptLocalSecret(
      ...
      LOCAL_SECRET_CONTEXTS.providerApiKey("OR_OUT"),
    );
    ...
    expect(runtime.apiKeySecret).toBeDefined();
    await expect(
      freshDecryptSecretEnvelope(
        runtime.apiKeySecret!,
        "provider:OpenAI Compatible",
      ),
    ).resolves.toBe("sk-or-v1-openrouter-key");
  });

  it("builds OpenAI Compatible BYOK envelopes for Omniroute presets", async () => {
    vi.resetModules();
    resetByokKeyMaterial();
    const {
      decryptSecretEnvelope: freshDecryptSecretEnvelope,
    ...
    const apiKeySecret = await encryptLocalSecret(
      "omniroute-local-key",
      LOCAL_SECRET_CONTEXTS.providerApiKey("OM_OUT"),
    );
    ...
    expect(runtime.type).toBe("OpenAI Compatible");
    expect(runtime.baseUrl).toBe("http://localhost:20128/v1");
    expect(runtime.apiKeySecret).toBeDefined();
    await expect(
      freshDecryptSecretEnvelope(
        runtime.apiKeySecret!,
    ...
    ).resolves.toBe("omniroute-local-key");
  });

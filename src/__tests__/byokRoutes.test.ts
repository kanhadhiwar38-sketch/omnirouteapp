...
describe("BYOK route integration — OpenAI Compatible presets", () => {
  beforeEach(() => {
    vi.resetModules();
    mocks.safeFetchJson.mockReset();
    mocks.safeFetchText.mockReset();
  ...
  it("fetches models from OpenRouter with the decrypted API key", async () => {
    mocks.resolveProviderRuntimeConfig.mockResolvedValue({
      type: "OpenAI Compatible",
    ...
    mocks.safeFetchJson.mockResolvedValue({
      response: new Response(null, { status: 200 }),
      data: {
        data: [{ id: "google/gemma-2b" }, { id: "meta-llama/llama-3-8b" }],
      },
    });
    ...
    expect(mocks.safeFetchJson).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({
          Authorization: "Bearer sk-or-v1-openrouter-key",
        }),
      }),
      expect.any(Object),
    );
    const body = await response.json();
    expect(body.models).toEqual(["google/gemma-2b", "meta-llama/llama-3-8b"]);
  });

  it("fetches models from Omniroute with the decrypted API key", async () => {
    mocks.resolveProviderRuntimeConfig.mockResolvedValue({
      type: "OpenAI Compatible",
    ...
    mocks.safeFetchJson.mockResolvedValue({
      response: new Response(null, { status: 200 }),
      data: {
        data: [{ id: "omniroute-gpt-4" }, { id: "omniroute-claude-3" }],
      },
    });
    ...
    expect(mocks.safeFetchJson).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({
          Authorization: "Bearer sk-omniroute-key",
        }),
      }),
      expect.any(Object),
    );
    const body = await response.json();
    expect(body.models).toEqual(["omniroute-gpt-4", "omniroute-claude-3"]);
  });

  it("returns 401 with provider type for OpenRouter when no API key is configured", async () => {
    mocks.resolveProviderRuntimeConfig.mockResolvedValue({
      type: "OpenAI Compatible",
      baseUrl: "https://openrouter.ai/api/v1",
    });
    ...
    expect(response.status).toBe(401);
    const body = await response.json();
    expect(body.error).toBe("OpenAI Compatible API key is not configured");
    expect(mocks.safeFetchJson).not.toHaveBeenCalled();
  });
});
